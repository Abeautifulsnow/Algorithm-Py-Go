...
function _create_folder_new() {
	...
	FileArray=("main.${fileSuffix}" README.md)
    for i in "${FileArray[@]}"; do
        i_path=${title_path}/${i}
...
function CreateFolder() {
	...
	[Aa][Ll][Ll])
		echo "************* Py *************"
		_create_folder_new "$title" "py" "./leetcode/${title}"
		echo "************* Go *************"
		_create_folder_new "$title" "go" "./leetcode/${title}"
		echo "Task done!!!"
		exit 1
		;;
    *)
	...
	_create_folder_new "$title" "$fileSuffix" "$titlePath"
